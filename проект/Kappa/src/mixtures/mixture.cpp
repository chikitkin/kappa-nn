...
//
//    diffusion_LHS.zeros((n_vibr_levels_total + num_atoms) * (n_vibr_levels_total + num_atoms), (n_vibr_levels_total + num_atoms) * (n_vibr_levels_total + num_atoms));
...
//}
//
//const arma::mat &kappa::Mixture::compute_diffusion_LHS(double T, kappa::models_omega model) {
//    double kT = K_CONST_K * T;
//
//    int i1, i2, i3, i4, k, offset=0;
//    for (i1=0; i1 < num_molecules; i1++) { // b
//        for (k=0; k < molecules[i1].num_vibr_levels[0]; k++) { // n
//            for (i2=0; i2<n_vibr_levels_total; i2++) { // c,i
//                for (i3=0; i3<n_vibr_levels_total; i3++) { // d,k
//                    diffusion_LHS(offset + i2, offset + i3) = thermal_conductivity_LHS.at(i2 * 3, i3 * 3); // TODO: replace with at()!
//                }
//                for (i3=0; i3<num_atoms; i3++) { // d
//                    diffusion_LHS(offset + i2, offset + n_vibr_levels_total + i3) = thermal_conductivity_LHS.at(i2 * 3, n_vibr_levels_total * 3 + i3 * 2); // TODO: replace with at()!
//                    diffusion_LHS(offset + n_vibr_levels_total + i3, offset + i2) = diffusion_LHS(offset + i2, offset + n_vibr_levels_total + i3); // TODO: replace with at()!
//                }
//            }
//            for (i2=0; i2<num_atoms; i2++) { // c
//                for (i3=0; i3<num_atoms; i3++) { // d
//                    diffusion_LHS(offset + n_vibr_levels_total + i2, offset + n_vibr_levels_total + i3) = thermal_conductivity_LHS.at(n_vibr_levels_total * 3 + i2 * 2, n_vibr_levels_total * 3 + i3 * 2); // TODO: replace with at()!
//                    diffusion_LHS(offset + n_vibr_levels_total + i3, offset + n_vibr_levels_total + i2) = diffusion_LHS(offset + n_vibr_levels_total + i2, offset + n_vibr_levels_total + i3); // TODO: replace with at()!
//                }
//            }
//            offset += n_vibr_levels_total + num_atoms;
//        }
//        // p1 = 3 * (vl_offset[i] + k);
...
#ifdef KAPPA_STRICT_CHECKScompute_transport_coefficients
	std::string error_string;
	if (T <= 0) {
		error_string = "Non-positive temperature: T=" + std::to_string(T);
		throw kappa::IncorrectValueException(error_string.c_str());
