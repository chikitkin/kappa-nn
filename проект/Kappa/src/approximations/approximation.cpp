...
double kappa::Approximation::Z_diss(double T, double U, const arma::vec &electron_energy, const arma::Col<unsigned long> &statistical_weight, int n_electron_levels,
...
double kappa::Approximation::Z_diss(double T, const arma::vec &electron_energy, const arma::Col<unsigned long> &statistical_weight, int n_electron_levels,
...
double kappa::Approximation::C_aliat(double T, const arma::vec &electron_energy, const arma::Col<unsigned long> &statistical_weight, int n_electron_levels,
                                     const std::vector<arma::vec> &vibr_energy, const std::vector<int> &num_vibr_levels,
                                     double vibr_energy_product, double activation_energy, int i, int e, double U) { // U != infty
  double tmp=0, res=0;
...
double kappa::Approximation::C_aliat(double T, const arma::vec &electron_energy, const arma::Col<unsigned long> &statistical_weight, int n_electron_levels,
                                     const std::vector<arma::vec> &vibr_energy, const std::vector<int> &num_vibr_levels,
                                     double vibr_energy_product, double activation_energy, int i, int e) { // U = infty
  double tmp=0, res=0;
...
double kappa::Approximation::p_avg_rot_energy(double T, const arma::vec &rot_energy, int num_rot_levels, int rot_symmetry) {
    return arma::dot((2 * arma::linspace<arma::vec>(0, num_rot_levels - 1, num_rot_levels) + 1) % rot_energy, arma::exp(-rot_energy / (K_CONST_K * T))) / (rot_symmetry * p_Z_rot(T, rot_energy, num_rot_levels, rot_symmetry));
}

double kappa::Approximation::p_avg_rot_energy_sq(double T, const arma::vec &rot_energy, int num_rot_levels, int rot_symmetry) {
    return arma::dot((2 * arma::linspace<arma::vec>(0, num_rot_levels - 1, num_rot_levels) + 1) % rot_energy % rot_energy, arma::exp(-rot_energy / (K_CONST_K * T))) / (rot_symmetry * p_Z_rot(T, rot_energy, num_rot_levels, rot_symmetry));
}
...
double kappa::Approximation::probability_VV_FHO(double rel_vel, double coll_mass, 
                                                double Delta_E_vibr,
  ...
  double avg_vel = vel_avg_vv(rel_vel, coll_mass, Delta_E_vibr);
    ...
    return pow(rhoxi, abs_delta) * ns12 * exp(-2.0 * rhoxi * pow(ns12, 1.0 / abs_delta) / (abs_delta + 1.0)) / (kappa::factorial_table[abs_delta] * kappa::factorial_table[abs_delta]);
}

double kappa::Approximation::probability_VT_FHO(double rel_vel, double coll_mass, double osc_mass, double Delta_E_vibr,
                                          int i, int delta_i,
    ...
    double eps = coll_mass / (sinh(K_CONST_PI * omega / (alpha_FHO * avg_vel)) * alpha_FHO);
    eps = eps * eps * omega / (4 * K_CONST_HBAR * s * osc_mass); // svt = 1 / (K_CONST_PI*s); TODO: 2 or 0.5???
  //  eps = eps * eps * omega * K_CONST_PI * 0.5 / (4 * K_CONST_HBAR * osc_mass); // svt = 1/ 9 ???
    return ns * pow(eps, s) * exp(- 2.0 * pow(ns, 1.0 / s) * eps / (s + 1.0)) / (kappa::factorial_table[s] * kappa::factorial_table[s]);
}
...
double kappa::Approximation::convert_vibr_ladder_O2(double vibr_energy) {
    ...
    return 0.4711499685975582 + 2.23383944 * te + 2.17784653 * te * te + 0.61279172 * te * te * te - 1.38809821 * te * te * te * te + 0.66364689 * te * te * te * te * te
           - 0.13147542 * te * te * te * te * te * te + 0.00950554 * te * te * te * te * te * te * te; 
}
...
double kappa::Approximation::crosssection_elastic_VSS(double rel_vel, double vss_c_cs, double vss_omega) {
  return vss_c_cs * pow(rel_vel, 2 * vss_omega - 1);
}

double kappa::Approximation::crosssection_VT_FHO_RS(double rel_vel, double coll_mass, double diameter, double osc_mass,
                                                    double Delta_E_vibr, int i, int delta_i,
                                                    double omega, double alpha_FHO) {
    if (delta_i < 0) {
    // std::cout << rel_vel << " " << Delta_E_vibr << " " << probability_VT_FHO(rel_vel, coll_mass, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO) << " " << omega << std::endl;
  //   std::cout << coll_mass << " " << osc_mass << " " << i << " " << delta_i << " " << alpha_FHO << std::endl;
      return probability_VT_FHO(rel_vel, coll_mass, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO) * crosssection_elastic_RS(diameter);
    }
    else {
      return 0.; // rewrite!
    }
...
double kappa::Approximation::crosssection_VT_FHO_VSS(double rel_vel, double coll_mass, double vss_c_cs, double vss_omega, double osc_mass,
                                                     double Delta_E_vibr, int i, int delta_i,
                                                     double omega, double alpha_FHO) {
    if (delta_i < 0) {
      return probability_VT_FHO(rel_vel, coll_mass, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO) * crosssection_elastic_VSS(rel_vel, vss_c_cs, vss_omega);
    }
    else {
      return 0.; // rewrite!
    }
...
double kappa::Approximation::crosssection_diss_ILT_N2N(double rel_vel, double coll_mass, double vibr_energy, double i) {
  ...
  return sqrt(K_CONST_PI * coll_mass / (8 * K_CONST_K)) * 7.16e-2 * (5.24e-19 * i * i * i - 7.41e-17 * i * i + 6.42e-15 * i + 7.3e-14) * pow(t - (c1 - vibr_energy) / K_CONST_K, 0.25) / (0.91 * t);
}
...
double kappa::Approximation::integral_VT_FHO_RS(double T, int degree, double coll_mass, double diameter, double osc_mass,
                        double Delta_E_vibr, int i, int delta_i, double omega, double alpha_FHO) {
  double conversion = sqrt(2 * K_CONST_K * T / coll_mass);
  auto integrand = [T, degree, coll_mass, diameter, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO, conversion](double g) {return pow(g, 2 * degree + 3) *
  crosssection_VT_FHO_RS(conversion * g, coll_mass, diameter, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO) * exp(-g * g); };
  return sqrt(K_CONST_K * T / (2 * K_CONST_PI * coll_mass)) * integrate_semi_inf(integrand, 0);
}

double kappa::Approximation::integral_VT_FHO_VSS(double T, int degree, double coll_mass, double vss_c_cs, double vss_omega, double osc_mass,
                         double Delta_E_vibr, int i, int delta_i,
                         double omega, double alpha_FHO) {
  double conversion = sqrt(2 * K_CONST_K * T / coll_mass);
  auto integrand = [T, degree, coll_mass, vss_c_cs, vss_omega, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO, conversion](double g) {return pow(g, 2 * degree + 3) *
  crosssection_VT_FHO_VSS(conversion * g, coll_mass, vss_c_cs, vss_omega, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO) * exp(-g * g); };
  return sqrt(K_CONST_K * T / (2 * K_CONST_PI * coll_mass)) * integrate_semi_inf(integrand, 0);
}

double kappa::Approximation::integral_diss_RS(double T, int degree, double coll_mass, double diameter, double diss_energy, double vibr_energy, bool center_of_mass) {
    double conversion = sqrt(2 * K_CONST_K * T / coll_mass);
    auto integrand = [T, degree, coll_mass, diameter, diss_energy, vibr_energy, center_of_mass, conversion](double g) {return pow(g, 2 * degree + 3) *
...
double kappa::Approximation::integral_diss_VSS(double T, int degree, double coll_mass, double vss_c_cs, double vss_omega, double diss_energy, double vibr_energy, bool center_of_mass) {
    double conversion = sqrt(2 * K_CONST_K * T / coll_mass);
...
double kappa::Approximation::P_SSH_VT_10(double T, double coll_mass, double omega_e, double epsilon, double diameter, double r_e) {
  ...
  double omega = 2 * K_CONST_PI * K_CONST_C * omega_e;
  double chi = pow(K_CONST_PI * K_CONST_PI * coll_mass * omega * omega / (2 * alpha * alpha * kT), 0.3333333);
  double r = diameter * pow(0.5 * sqrt(1 + chi * kT / epsilon) + 0.5, -0.1666666);
  double Z0 = alpha * r_e * alpha * r_e *exp((-3 * alpha * r_e * r_e) / (8 * r));
  return 1.294 * pow(r / diameter, 2) / (Z0 * alpha * alpha * K_CONST_HBAR * (1 + 1.1 * epsilon / kT)) * 4 * K_CONST_PI * K_CONST_PI * coll_mass * omega
         * sqrt(4 * K_CONST_PI * chi / 3) * exp(-3 * chi + K_CONST_HBAR * omega / (2 * kT) + epsilon /kT);
}
...
double kappa::Approximation::k_VT_SSH(double T, int i, double coll_mass, double diameter, double omega_e, double epsilon, double r_e) {
  return i*p_Z_coll(T, 1., coll_mass, diameter) * P_SSH_VT_10(T, coll_mass, omega_e, epsilon, diameter, r_e);
}

double kappa::Approximation::k_VT_SSH(double T, int i, double coll_mass, double diameter, double omega_e, double epsilon, double r_e, double Delta_E_vibr, double vibr_energy_1) {
  double alpha = 17.5 / diameter;
...
double kappa::Approximation::k_VT_FHO_RS(double T, double coll_mass, double diameter, double osc_mass,
                                         double Delta_E_vibr, int i, int delta_i, double omega, double alpha_FHO) {
  return 8 * integral_VT_FHO_RS(T, 0, coll_mass, diameter, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO);
}

double kappa::Approximation::k_VT_FHO_VSS(double T, double coll_mass, double vss_c_cs, double vss_omega, double osc_mass,
                                          double Delta_E_vibr, int i, int delta_i,
                                          double omega, double alpha_FHO) {
  return 8 * integral_VT_FHO_VSS(T, 0, coll_mass, vss_c_cs, vss_omega, osc_mass, Delta_E_vibr, i, delta_i, omega, alpha_FHO);
}
...
double kappa::Approximation::k_exch_WRFP(double T, double vibr_energy, double activation_energy,
                                         double alpha_exch, double beta_exch, double A_exch, double n_exch) {
  if (activation_energy > alpha_exch * vibr_energy) {
    return A_exch * pow(T, n_exch) * exp(-(activation_energy - alpha_exch * vibr_energy) / (K_CONST_K * T * beta_exch)); //alpha_exch-?
  }
  else
  {
...
double kappa::Approximation::p_vibr_relaxation_time_MW(double T, double n, double char_vibr_temp, double coll_mass) {
  ...
  return pow(10.0, ((5e-4 * pow(md, 0.5) * pow(char_vibr_temp, 1.333333333) * (pow(T, -0.3333333) - 0.015 * pow(md, 0.25))) - 8)) / (n * K_CONST_K * T * 9.8692326671601e-6) ;
}
...
// Returns the \Omega^{(l,r)}-integral for a rigid sphere potential for any l > 0 and r > 0
    return sqrt(T * K_CONST_K / (2 * K_CONST_PI * coll_mass)) * 0.5 * kappa::factorial_table[r + 1] * (1.0 - 0.5 * (1.0 + pow(-1.0, l)) / (l + 1)) * K_CONST_PI * (diameter * diameter);
}
...
// Returns the \Omega^{(l,r)}-integral for the VSS potential
  ...
  switch (l) {
    ...
    case 3:
      ...
      break;
    case 4:
      mult = (4 * vss_alpha * vss_alpha * vss_alpha + 12 * vss_alpha * vss_alpha + 32 * vss_alpha) / ((4 + vss_alpha) * (3 + vss_alpha) * (2 + vss_alpha) * (1 + vss_alpha));
      break;
    case 5:
      mult = (5 * vss_alpha * vss_alpha * vss_alpha * vss_alpha
              + 30 * vss_alpha * vss_alpha * vss_alpha + 115 * vss_alpha * vss_alpha + 90 * vss_alpha + 120) / ((5 + vss_alpha) * (4 + vss_alpha) * (3 + vss_alpha) * (2 + vss_alpha) * (1 + vss_alpha));
      break;
    default:
      return -1;
      break; 
...
// Returns the \Omega^{(l,r)}-integral for the Lennard-Jones potential
  if (r == 1 && l == 1) {
    ...
    return 1. / (-0.16845 - 0.02258 / (log_KT_eps * log_KT_eps) + 0.19779 / log_KT_eps + 0.64373 * log_KT_eps - 0.09267 * log_KT_eps * log_KT_eps + 0.00711 * log_KT_eps * log_KT_eps * log_KT_eps);
    //  Kustova-Nagnibeda (5.99)
  }
  else if (r == 2 && l == 2)
  {
    double log_KT_eps = log(K_CONST_K * T / epsilon) + 1.5;
    return 1. / (-0.40811 - 0.05086 / (log_KT_eps * log_KT_eps) + 0.34010 / log_KT_eps + 0.70375 * log_KT_eps - 0.10699 * log_KT_eps * log_KT_eps + 0.00763 * log_KT_eps * log_KT_eps * log_KT_eps);
    //  Kustova-Nagnibeda (5.99)
  }
  else {
    return -1.;
...
double kappa::Approximation::omega_integral_Born_Mayer(double T, int l, int r, double beta, double phi_zero, double diameter, double epsilon) {
  if ((r == 1 && l == 1) || (r == 2 && l == 2)) {
    ...
    for (int i = 0; i<6; i++)
    {
      Born_Mayer_A[i] = Born_Mayer_coeff_array[i][0] + pow(log_v_star  / (beta * diameter), -2) * (Born_Mayer_coeff_array[i][1] + Born_Mayer_coeff_array[i][2] / log_v_star
                                                       + Born_Mayer_coeff_array[i][3] / (log_v_star * log_v_star));
      // coefficients for Born-Mayer potential, Kustova-Nagnibeda (5.100)
    }
    if (r == 1 && l == 1)
    {
      return pow(log(phi_zero / (K_CONST_K * T)) / (beta * diameter), 2) * (0.89 + Born_Mayer_A[0] / (KT_eps * KT_eps) + Born_Mayer_A[1] / pow(KT_eps, 4) + Born_Mayer_A[2] / pow(KT_eps, 6));
    }
    else // r == 2 and l == 2
    {
      KT_eps = log(KT_eps);
      return pow(log(phi_zero / (K_CONST_K * T)) / (beta * diameter), 2) * (1.04 + Born_Mayer_A[3] / (KT_eps * KT_eps) + Born_Mayer_A[4] / (KT_eps * KT_eps * KT_eps) + Born_Mayer_A[5] / pow(KT_eps, 4));
    }
  }
  else {
    return -1.;
...
double kappa::Approximation::omega_integral_ESA_nn(double T, int l, int r, double beta, double epsilon_zero, double r_e) {
  double m = 6;  // parameter for neutral-neutral interactions ESA STR 256 Table 4 [dimensionless]
  double x_0 = 0.8002 * pow(beta, 0.049256);
  double sigma_sqr = x_0 * x_0 * r_e * r_e * 1e20;
  double x = log(K_CONST_K * T / epsilon_zero);
  if (l == 1 && r == 1) {
    double a[7] = { (7.884756e-1) - (2.438494e-2) * beta, -2.952759e-1 - (1.744149e-3) * beta, (5.020892e-1) + (4.316985e-2) * beta, (-9.042460e-1) - (4.017103e-2) * beta,
    (-3.373058) + (2.458538e-1) * beta - (4.850047e-3) * beta * beta, (4.161981) + (2.202737e-1) * beta - (1.718010e-2) * beta * beta, (2.462523) + (3.231308e-1) * beta - (2.281072e-2) * beta * beta };
    ...
    double a[7] = { (7.123565e-1) + (-2.688875e-2) * beta, (-2.9105e-1) + (-2.065175e-3) * beta, (4.187065e-2) + (4.060236e-2) * beta, (-9.287685e-1) + (-2.342270e-2) * beta,
    (-3.598542) + (2.54512e-1) * beta + (-4.685966e-3) * beta * beta, (3.934824) + (2.699944e-1) * beta + (-2.009886e-2) * beta * beta, 2.578084 + (3.449024e-1) * beta + (-2.2927e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (6.606022e-1) + (-2.831448e-2) * beta, (-2.8709e-1) + (-2.232827e-3) * beta, (-2.51969e-1) + (3.778211e-2) * beta, (-9.173046e-1) + (-1.864476e-2) * beta,
    (-3.776812) + (2.552528e-1) * beta + (-4.23722e-3) * beta * beta, (3.768103) + (3.155025e-1) * beta + (-2.218849e-2) * beta * beta, 2.695440 + (3.597998e-1) * beta + (-2.267102e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (6.268016e-1) + (-2.945078e-2) * beta, (-2.830834e-1) + (-2.361273e-3) * beta, (-4.559927e-1) + (3.705640e-2) * beta, (-9.334638e-1) + (-1.797329e-2) * beta,
    (-3.947019) + (2.446843e-1) * beta + (-3.176374e-3) * beta * beta, (3.629926) + (3.761272e-1) * beta + (-2.451016e-2) * beta * beta, 2.824905 + (3.781709e-1) * beta + (-2.251978e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (5.956859e-1) + (-2.915893e-2) * beta, (-2.804989e-1) + (-2.298968e-3) * beta, (-5.965551e-1) + (3.724395e-2) * beta, (-8.946001e-1) + (-2.550731e-2) * beta,
    (-4.076798) + (1.983892e-1) * beta + (-5.014065e-3) * beta * beta, (3.458362) + (4.770695e-1) * beta + (-2.678054e-2) * beta * beta, 2.982260 + (4.014572e-1) * beta + (-2.142580e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (7.898524e-1) - (2.114115e-2) * beta, (-2.998325e-1) - (1.243977e-3) * beta, (7.077103e-1) + (3.583907e-2) * beta, (-8.946857e-1) - (2.473947e-2) * beta,
    (-2.958969) + (2.303358e-1) * beta - (5.226562e-3) * beta * beta, (4.348412) + (1.920321e-1) * beta - (1.496557e-2) * beta * beta, (2.205440) + (2.567027e-1) * beta - (1.861359e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (7.269006e-1) - (2.233866e-2) * beta, (-2.972304e-1) - (1.392888e-3) * beta, (3.904230e-1) + (3.231655e-2) * beta, (-9.442201e-1) - (1.494805e-2) * beta,
    (-3.137828) + (2.347767e-1) * beta - (4.963979e-3) * beta * beta, (4.190370) + (2.346004e-1) * beta - (1.718963e-2) * beta * beta, (2.319751) + (2.700236e-1) * beta - (1.854217e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (6.829159e-1) - (2.233866e-2) * beta, (-2.943232e-1) - (1.514322e-3) * beta, (1.414623e-1) + (3.075351e-2) * beta, (-9.720228e-1) - (1.038869e-2) * beta,
    (-3.284219) + (2.243767e-1) * beta - (3.913041e-3) * beta * beta, (4.011692) + (3.005083e-1) * beta - (2.012373e-2) * beta * beta, (2.401249) + (2.943600e-1) * beta - (1.884503e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (7.468781e-1) - (2.518134e-2) * beta, (-2.947438e-1) - (1.811571e-3) * beta, (2.234096e-1) + (3.681114e-2) * beta, (-9.974591e-1) - (2.670805e-2) * beta,
    (-3.381787) + (2.372932e-1) * beta - (4.239629e-3) * beta * beta, (4.094540) + (2.756466e-1) * beta - (2.009227e-2) * beta * beta, (2.476087) + (3.300898e-1) * beta - (2.223317e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
    ...
    double a[7] = { (7.365470e-1) - (2.242357e-2) * beta, (-2.968650e-1) - (1.396696e-3) * beta, (3.747555e-1) + (2.847063e-2) * beta, (-9.944036e-1) - (1.378926e-2) * beta,
    (-3.136655) + (2.176409e-1) * beta - (3.899247e-3) * beta * beta, (4.145871) + (2.855836e-1) * beta - (1.939452e-2) * beta * beta, (2.315532) + (2.842981e-1) * beta - (1.874462e-2) * beta * beta };
    double exp_1 = exp((x - a[2]) / a[3]);
    double exp_2 =  exp((x - a[5]) / a[6]);
    // return exp((a[0] + a[1] * x) * exp((x - a[2]) / a[3]) / (exp((x - a[2]) / a[3]) + exp((a[2] - x) / a[3])) + a[4] * exp((x - a[5]) / a[6]) / (exp((x - a[5]) / a[6]) + exp((a[5] - x) / a[6]))) * sigma_sqr;
    return exp((a[0] + a[1] * x) * exp_1 / (exp_1 + 1 / exp_1) + a[4] * exp_2 / (exp_2 + 1 / exp_2));
...
double kappa::Approximation::omega_integral_ESA_cn(double T, int l, int r, double beta, double epsilon_zero, double r_e) {
  ...
  double sigma_sqr = x0 * x0 * r_e * r_e; // value, to obtain dimensional  \Omega^{(l,r)}-integral [m]; 
  double x = log(K_CONST_K * T / epsilon_zero);
  if (l == 1 && r == 1) {
    double a[7] = { 
...
double kappa::Approximation::omega_integral_ESA_ne(double T, double g1, double g2, double g3, double g4, double g5, double g6, double g7, double g8, double g9, double g10, double diameter) {
  double x = log(T);
  double exp_1 = exp(x - g1 / g2);

  return (g3 * pow(x, g6) * exp_1 / (exp_1 + 1. / exp_1) + g7 * exp(-(x - g8) * (x - g8) / (g9 * g9)) + g4 + g10 * pow(x, g5)) / (1e20 * diameter * diameter);
}

double kappa::Approximation::p_k_bf_VT(double T, double vibr_energy_before, double vibr_energy_after, const arma::vec &rot_energy_before, int num_rot_levels_before, 
  ...
  if (is_rigid_rotator) {
    ...
    return exp((vibr_energy_after - vibr_energy_before) / (K_CONST_K * T)) * p_Z_rot(T, rot_energy_before, num_rot_levels_before, rot_symmetry) /  p_Z_rot(T, rot_energy_after, num_rot_levels_after, rot_symmetry);
  }
}


double kappa::Approximation::p_k_bf_VV(double T, double vibr_energy1_before, double vibr_energy1_after, double vibr_energy2_before, double vibr_energy2_after,
    ...
    return exp((vibr_energy1_after - vibr_energy1_before + vibr_energy2_after - vibr_energy2_before) / (K_CONST_K * T))
           * p_Z_rot(T, rot_energy1_before, num_rot_levels1_before, rot_symmetry1) * p_Z_rot(T, rot_energy2_before, num_rot_levels2_before, rot_symmetry2)
           / (p_Z_rot(T, rot_energy1_after, num_rot_levels1_after, rot_symmetry1) * p_Z_rot(T, rot_energy2_after, num_rot_levels2_after, rot_symmetry2));
}

double kappa::Approximation::p_k_bf_exch(double T, double molecule_before_mass, double atom_before_mass, double molecule_after_mass, double atom_after_mass,
  ...
  return pow(molecule_before_mass * atom_before_mass / (molecule_after_mass * atom_after_mass), 1.5)
         * (p_Z_rot(T, rot_energy_before, num_rot_levels_before, rot_symmetry_before) / p_Z_rot(T, rot_energy_after, num_rot_levels_after, rot_symmetry_after))
         * exp((vibr_energy_after - vibr_energy_before) / (K_CONST_K * T))
         * exp((diss_energy_before - diss_energy_after) / (K_CONST_K * T));
}

double kappa::Approximation::p_k_bf_diss(double T, double molecule_mass, double atom1_mass, double atom2_mass, double diss_energy, double vibr_energy,
                                       const arma::vec &rot_energy, int num_rot_levels, int rot_symmetry) {
  return pow(molecule_mass / (atom1_mass * atom2_mass), 1.5) * K_CONST_H * K_CONST_H * K_CONST_H * pow(2 * K_CONST_PI * K_CONST_K * T, -1.5)
                                                             * p_Z_rot(T, rot_energy, num_rot_levels, rot_symmetry) * exp((diss_energy - vibr_energy) / (K_CONST_K * T));
}
...
double kappa::Approximation::Z_rot(double T, const kappa::Molecule &molecule, int i, int e) {
  ...
  if (e > molecule.num_electron_levels-1) {
    std::string error_string = "Electron level larger than maximum level: e=" + std::to_string(e) + ", maximum level=" + std::to_string(molecule.num_electron_levels-1);
    throw kappa::IncorrectValueException(error_string.c_str());
  }
  if (i > molecule.num_vibr_levels[e]-1) {
    std::string error_string = "Vibrational level larger than maximum level: i=" + std::to_string(i) + ", maximum level=" + std::to_string(molecule.num_vibr_levels[e]-1);
    throw kappa::IncorrectValueException(error_string.c_str());
  }
  #endif
...
double kappa::Approximation::probability_VV(double rel_vel, kappa::Molecule const &molecule1, kappa::Molecule const &molecule2, kappa::Interaction const &interaction,
  ...
  if (((interaction.particle1_name != molecule1.name) || (interaction.particle2_name != molecule2.name)) && ((interaction.particle1_name != molecule2.name) || (interaction.particle2_name != molecule1.name))) {
    std::string error_string = "Interaction is for a different set of particles, the interaction is for: " + interaction.particle1_name + " + " + interaction.particle2_name;
...
double kappa::Approximation::probability_VT(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction,
                                            int i, int delta_i, int e, kappa::models_prob_vt model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (rel_vel < 0.0) {
    std::string error_string = "Negative relative velocity: rel_vel=" + std::to_string(rel_vel);
    throw kappa::IncorrectValueException(error_string.c_str());
  ...
  if (model == models_prob_vt::model_prob_vt_fho) {
    ...
    return probability_VT_FHO(rel_vel, interaction.collision_mass, molecule.reduced_osc_mass,
                              molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                              i, delta_i, omega, interaction["alpha_FHO"]);
  }
  else {
...
double kappa::Approximation::probability_diss(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int e, kappa::models_prob_diss model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (rel_vel < 0.0) {
    std::string error_string = "Negative relative velocity: rel_vel=" + std::to_string(rel_vel);
    throw kappa::IncorrectValueException(error_string.c_str());
...
double kappa::Approximation::probability_diss(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, kappa::models_prob_diss model) {
  ...
  switch (model) {
    case models_cs_elastic::model_cs_el_vss:
      if (interaction.vss_data) {
        return crosssection_elastic_VSS(rel_vel, interaction.collision_mass, interaction.vss_c_cs, interaction.vss_omega);
      }
      else {
...
double kappa::Approximation::crosssection_VT(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction,
  ...
  switch(model) {
    case models_cs_vt::model_cs_vt_rs_fho: {
      double omega = abs(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * abs(delta_i));
      return crosssection_VT_FHO_RS(rel_vel, interaction.collision_mass, interaction.collision_diameter, molecule.reduced_osc_mass,
                                    molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                                    i, delta_i, omega, interaction["alpha_FHO"]);
      break;
    }
    case models_cs_vt::model_cs_vt_vss_fho: {
      double omega = abs(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * abs(delta_i));
      if (interaction.vss_data) {
        return crosssection_VT_FHO_VSS(rel_vel, interaction.collision_mass, interaction.vss_c_cs, interaction.vss_omega, molecule.reduced_osc_mass,
                                       molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                                       i, delta_i, omega, interaction["alpha_FHO"]);
      }
      else {
...
double kappa::Approximation::crosssection_diss(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int e, kappa::models_cs_diss model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (rel_vel < 0.0) {
    std::string error_string = "Negative relative velocity: rel_vel=" + std::to_string(rel_vel);
    throw kappa::IncorrectValueException(error_string.c_str());
...
double kappa::Approximation::crosssection_diss(double rel_vel, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, kappa::models_cs_diss model) {
...
double kappa::Approximation::rot_relaxation_time_parker(double T, double n, kappa::Molecule const &molecule, kappa::Interaction const &interaction, kappa::models_omega model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (T <= 0) {
    std::string error_string = "Non-positive temperature: T=" + std::to_string(T);
    throw kappa::IncorrectValueException(error_string.c_str());
  ...
  return p_rot_collision_number_parker(T, molecule.parker_const, molecule.LennardJones_epsilon) * K_CONST_PI * 0.15625 / (n * omega_integral(T, interaction, 2, 2, model, true));
}
  ...
  switch (model) {
    case models_omega::model_omega_rs:
        ...
        if (interaction.vss_data) {
          if ((l>5)) {
            ...
            if (dimensional) {
              ...
              return omega_integral_VSS(T, l, r, interaction.collision_mass, interaction.vss_c_cs, interaction.vss_omega, interaction.vss_alpha) / omega_integral_RS(T, l, r, interaction.collision_diameter, interaction.collision_mass);
            }
          }
        }
        else {
      ...
      case models_omega::model_omega_lennardjones:
        ...
        r0 = T * T * (rp1 - 2 * r0 + rm1) / (K_CONST_OMEGA_D_STEP_SIZE * K_CONST_OMEGA_D_STEP_SIZE)
              ...
              double res =  (omega_integral_Born_Mayer(T+K_CONST_OMEGA_D_STEP_SIZE, l, l, interaction["beta Born-Mayer"],
                             ...
                             * omega_integral_RS(T+K_CONST_OMEGA_D_STEP_SIZE, l, l, interaction.collision_diameter, interaction.collision_mass) -
                             omega_integral_Born_Mayer(T-K_CONST_OMEGA_D_STEP_SIZE, l, l, interaction["beta Born-Mayer"],
                                                       interaction["phi_zero Born-Mayer"], interaction.collision_diameter, interaction["Lennard-Jones epsilon"])
                             * omega_integral_RS(T-K_CONST_OMEGA_D_STEP_SIZE, l, l, interaction.collision_diameter, interaction.collision_mass)) * T / (2 * K_CONST_OMEGA_D_STEP_SIZE) +
                            (l + 1.5) * omega_integral_Born_Mayer(T, l, l, interaction["beta Born-Mayer"],
                                                                  interaction["phi_zero Born-Mayer"], interaction.collision_diameter, interaction["Lennard-Jones epsilon"])
                            * omega_integral_RS(T, l, l, interaction.collision_diameter, interaction.collision_mass);
              if (dimensional == true) {
                  return res;
      ...
      case models_omega::model_omega_esa:
        if (interaction.interaction_type == interaction_types::interaction_neutral_neutral) {
          ...
          if ((l == 1 && r == 1) || (l == 1 && r == 2) || (l == 1 && r == 3) || (l == 1 && r == 4) || (l == 1 && r == 5) || (l == 2 && r == 2) || (l == 2 && r == 3) || (l == 2 && r == 4) || (l == 3 && r == 3) || (l == 4 && r == 4)) {
            // double x0 = 0.7564 * pow(interaction["beta Bruno"], 0.064605);
            // double sigma_sqr = x0 * x0 * interaction["re Bruno"] * interaction["re Bruno"];
            ...
            if (dimensional == true) {
              ...
              double beta = interaction["beta Bruno"];
              double r_e = interaction["re Bruno"];
              double x_0 = 0.7564 * pow(beta, 0.064605);
              double sigma_sqr = x_0 * x_0 * r_e * r_e * 1e20;

              return tmp * sigma_sqr;
            }
          }
          else {
            std::string error_string = "Incorrect values of (l, r) specified: (" + std::to_string(l) + ", " + std::to_string(r) + ");" +
          ...
          if ((l == 1 && r == 1) || (l == 1 && r == 2) || (l == 1 && r == 3) || (l == 1 && r == 4) || (l == 1 && r == 5) || (l == 2 && r == 2) || (l == 2 && r == 3) || (l == 2 && r == 4) || (l == 3 && r == 3)) {
            ...
            if (dimensional == true) {
              return omega_integral_ESA_ne(T, interaction[lrc+"0"], interaction[lrc+"1"], interaction[lrc+"2"], interaction[lrc+"3"], interaction[lrc+"4"], interaction[lrc+"5"],
                                           interaction[lrc+"6"], interaction[lrc+"7"], interaction[lrc+"8"], interaction[lrc+"9"],
                                           interaction.collision_diameter) * omega_integral_RS(T, l, r, interaction.collision_diameter, interaction.collision_mass);
            }
            else {
              return omega_integral_ESA_ne(T, interaction[lrc+"0"], interaction[lrc+"1"], interaction[lrc+"2"], interaction[lrc+"3"], interaction[lrc+"4"], interaction[lrc+"5"],
                                           interaction[lrc+"6"], interaction[lrc+"7"], interaction[lrc+"8"], interaction[lrc+"9"],
...
double kappa::Approximation::omega_integral(double T, kappa::Interaction const &interaction, int l, int r, double debye_length, kappa::models_omega model, bool dimensional) {
  ...
  if (interaction.interaction_type != interaction_types::interaction_charged_charged) {
    ...
    if ((l == 1 && r == 1) || (l == 1 && r == 2) || (l == 1 && r == 3) || (l == 1 && r == 4) || (l == 1 && r == 5) || (l == 2 && r == 2) || (l == 2 && r == 3) || (l == 2 && r == 4) || (l == 3 && r == 3)) {
      if (dimensional) {
        return omega_integral_ESA_cc(T, l, r, interaction.charge1, interaction.charge2, debye_length) * omega_integral_RS(T, l, r, interaction.collision_diameter, interaction.collision_mass);
      }
      else {
...
double kappa::Approximation::integral_VT(double T, int degree, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int delta_i, int e, kappa::models_cs_vt model) {
  ...
  switch (model) {
    case models_cs_vt::model_cs_vt_rs_fho: {
      double omega = abs(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * abs(delta_i));
      return integral_VT_FHO_RS(T, degree, interaction.collision_mass, interaction.collision_diameter, molecule.reduced_osc_mass,
                  molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                i, delta_i, omega, interaction["alpha_FHO"]);
      break;
    }
    case models_cs_vt::model_cs_vt_vss_fho: {
      if (interaction.vss_data) {
        double omega = abs(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * abs(delta_i));
        return integral_VT_FHO_VSS(T, degree, interaction.collision_mass, interaction.vss_c_cs, interaction.vss_omega, molecule.reduced_osc_mass,
                   molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                   i, delta_i, omega, interaction["alpha_FHO"]);
      }
      else {
...
double kappa::Approximation::k_VT(double T, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int delta_i, int e, kappa::models_k_vt model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (T <= 0) {
    std::string error_string = "Non-positive temperature: T=" + std::to_string(T);
    throw kappa::IncorrectValueException(error_string.c_str());
  ...
  switch (model) {
    case models_k_vt::model_k_vt_rs_fho: {
      ...
      double omega = -(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * delta_i);
          return k_VT_FHO_RS(T, interaction.collision_mass, interaction.collision_diameter, molecule.reduced_osc_mass,
                             molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                             i, delta_i, omega, interaction["alpha_FHO"]);
      }
          ...
          return k_VT_FHO_RS(T, interaction.collision_mass, interaction.collision_diameter, molecule.reduced_osc_mass,
                             ...
                             i + delta_i, -delta_i, omega, interaction["alpha_FHO"]) * k_bf_VT(T, molecule, i + delta_i, -delta_i, e);
      }
      break;
    }
    case models_k_vt::model_k_vt_vss_fho: {
      ...
      if (delta_i < 0) {
        double omega = -(molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i]) / (K_CONST_HBAR * delta_i);
        return k_VT_FHO_VSS(T, interaction.collision_mass, interaction.vss_c_cs, interaction.vss_omega, molecule.reduced_osc_mass,
                            molecule.vibr_energy[e][i] - molecule.vibr_energy[e][i + delta_i],
                            i, delta_i, omega, interaction["alpha_FHO"]);
      }
      else {
    ...
    case models_k_vt::model_k_vt_ssh:
      ...
      double Delta_E_vibr = K_CONST_H * K_CONST_C * molecule.vibr_we_xe[0];
          return k_VT_SSH(T, i, interaction.collision_mass, interaction.collision_diameter, molecule.vibr_frequency[0], interaction.epsilon, molecule.internuclear_distance,
             Delta_E_vibr, molecule.vibr_energy[0][1] - molecule.vibr_energy[0][0]);
        }
        else {
          return k_VT_SSH(T, i, interaction.collision_mass, interaction.collision_diameter, molecule.vibr_frequency[0], interaction.epsilon, molecule.internuclear_distance);
        }
        ...
        if (molecule.anharmonic_spectrum) {
      double Delta_E_vibr = K_CONST_H * K_CONST_C * molecule.vibr_we_xe[0];
          return k_VT_SSH(T, i + delta_i, interaction.collision_mass, interaction.collision_diameter, molecule.vibr_frequency[0], interaction.epsilon, molecule.internuclear_distance,
            Delta_E_vibr, molecule.vibr_energy[0][1] - molecule.vibr_energy[0][0]) * k_bf_VT(T, molecule, i + delta_i, -delta_i, 0);
        }
        else {
          return k_VT_SSH(T, i + delta_i, interaction.collision_mass, interaction.collision_diameter, molecule.vibr_frequency[0], interaction.epsilon, molecule.internuclear_distance) * k_bf_VT(T, molecule, i + delta_i, -delta_i, 0);
        }
      }
      else {
    ...
    case models_k_vt::model_k_vt_billing: {
      ...
      else if (((interaction.particle1_name == "N2") && (interaction.particle2_name == "N")) || ((interaction.particle1_name == "N") && (interaction.particle2_name == "N2"))) {
        if (delta_i < 0) {
          ...
          std::string error_string = "No Billing approximation for VT rate for the following species: " + interaction.particle1_name + ", " + interaction.particle2_name;
          throw kappa::ModelParameterException(error_string.c_str());
      }
    }
    case models_k_vt::model_k_vt_phys4entry: {
      ...
      if (((interaction.particle1_name == "N2") && (interaction.particle2_name == "N")) || ((interaction.particle1_name == "N") && (interaction.particle2_name == "N2"))) {
        ...
        if (delta_i < 0) {
          ...
          std::string error_string = "No Phys4Entry approximation for VT rate for the following species: " + interaction.particle1_name + ", " + interaction.particle2_name;
          throw kappa::ModelParameterException(error_string.c_str());
      }
    }
    default:
...
double kappa::Approximation::k_VT(double T, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int delta_i, kappa::models_k_vt model) {
  return k_VT(T, molecule, interaction, i, delta_i, 0, model);
}

double kappa::Approximation::k_exch(double T, kappa::Molecule const &molecule, kappa::Atom const &atom, kappa::Interaction const &interaction,
                                    int i, int e, int num_electron_levels, kappa::models_k_exch model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (T <= 0) {
    std::string error_string = "Non-positive temperature: T=" + std::to_string(T);
    throw kappa::IncorrectValueException(error_string.c_str());
  ...
  if (((interaction.particle1_name != molecule.name) || (interaction.particle2_name != atom.name)) && ((interaction.particle1_name != atom.name) || (interaction.particle2_name != molecule.name))) {
    std::string error_string = "Interaction is for a different set of particles, the interaction is for: " + interaction.particle1_name + " + " + interaction.particle2_name;
  ...
  switch (model) {
    case models_k_exch::model_k_exch_arrh_scanlon:
      if (((molecule.name=="N2") && (atom.name=="O")) || ((molecule.name=="O2") && (atom.name=="N")) || ((molecule.name=="NO") && (atom.name=="O")) ||((molecule.name=="NO") && (atom.name=="N"))) {
    ...
    case models_k_exch::model_k_exch_polak:
      if (((molecule.name=="N2") && (atom.name=="O")) || ((molecule.name=="O2") && (atom.name=="N"))) {
        return k_exch_WRFP(T, molecule.vibr_energy[e][i], interaction["exch,Ea,WRFP"], interaction["exch,alpha,Polak"], interaction["exch,beta,Polak"],
...
double kappa::Approximation::k_exch(double T, kappa::Molecule const &molecule, kappa::Atom const &atom, kappa::Interaction const &interaction, int i, kappa::models_k_exch model) {
  return k_exch(T, molecule, atom, interaction, i, 0, 1, model);
}

double kappa::Approximation::k_exch(double T, kappa::Molecule const &molecule, kappa::Atom const &atom, kappa::Molecule const &molecule_prod, kappa::Interaction const &interaction,
  ...
  switch (model) {
    case models_k_exch::model_k_exch_maliat_D6k_arrh_scanlon:
      if (((molecule.name=="N2") && (atom.name=="O")) || ((molecule.name=="O2") && (atom.name=="N")) || ((molecule.name=="NO") && (atom.name=="O")) ||((molecule.name=="NO") && (atom.name=="N"))) {
        return k_Arrhenius(T, interaction["exch,Arrh_A,Scanlon"], interaction["exch,Arrh_n,Scanlon"], interaction["exch,Ea,Scanlon"])
               * C_aliat(T, molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                         molecule.vibr_energy, molecule.num_vibr_levels, molecule_prod.vibr_energy[0][k], interaction["exch,Ea,Scanlon"], i, e, molecule.diss_energy[e] / (6 * K_CONST_K));
      }
      else  {
        std::string error_string = "No Scanlon data for exchange reaction for the following species: " + molecule.name + ", " + atom.name;
        throw kappa::ModelParameterException(error_string.c_str());
    ...
    case models_k_exch::model_k_exch_maliat_3T_arrh_scanlon:
      if (((molecule.name=="N2") && (atom.name=="O")) || ((molecule.name=="O2") && (atom.name=="N")) || ((molecule.name=="NO") && (atom.name=="O")) ||((molecule.name=="NO") && (atom.name=="N"))) {
        return k_Arrhenius(T, interaction["exch,Arrh_A,Scanlon"], interaction["exch,Arrh_n,Scanlon"], interaction["exch,Ea,Scanlon"])
               * C_aliat(T, molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                         molecule.vibr_energy, molecule.num_vibr_levels, molecule_prod.vibr_energy[0][k], interaction["exch,Ea,Scanlon"], i, e, 3 * T);
      }
      else  {
        std::string error_string = "No Scanlon data for exchange reaction for the following species: " + molecule.name + ", " + atom.name;
        throw kappa::ModelParameterException(error_string.c_str());
    ...
    case models_k_exch::model_k_exch_maliat_infty_arrh_scanlon:
      if (((molecule.name=="N2") && (atom.name=="O")) || ((molecule.name=="O2") && (atom.name=="N")) || ((molecule.name=="NO") && (atom.name=="O")) ||((molecule.name=="NO") && (atom.name=="N"))) {
        return k_Arrhenius(T, interaction["exch,Arrh_A,Scanlon"], interaction["exch,Arrh_n,Scanlon"], interaction["exch,Ea,Scanlon"])
               * C_aliat(T, molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                         molecule.vibr_energy, molecule.num_vibr_levels, molecule_prod.vibr_energy[0][k], interaction["exch,Ea,Scanlon"], i, e);
      }
      else  {
        std::string error_string = "No Scanlon data for exchange reaction for the following species: " + molecule.name + ", " + atom.name;
        throw kappa::ModelParameterException(error_string.c_str());
...
double kappa::Approximation::integral_diss(double T, int degree, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, kappa::models_cs_diss model) {
  return integral_diss(T, degree, molecule, interaction, i, 0, model);
}

double kappa::Approximation::k_diss(double T, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, int e, int num_electron_levels, kappa::models_k_diss model) {
  #ifdef KAPPA_STRICT_CHECKS
  if (T <= 0) {
    std::string error_string = "Non-positive temperature: T=" + std::to_string(T);
    throw kappa::IncorrectValueException(error_string.c_str());
  ...
  switch (model) {
   ...
   case models_k_diss::model_k_diss_rs_thresh_cmass:
    ...
    case models_k_diss::model_k_diss_arrh_scanlon:
      return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]);
      break;
    case models_k_diss::model_k_diss_arrh_park:
      return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Park"], interaction["diss," + molecule.name + ",Arrh_n,Park"], interaction["diss," + molecule.name + ",Ed,Park"]);
      break;
    case models_k_diss::model_k_diss_tm_D6k_arrh_scanlon:
        if (num_electron_levels == 1) {
            ...
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, molecule.diss_energy[e] / (6 * K_CONST_K), molecule.electron_energy, molecule.statistical_weight, molecule.num_electron_levels,
                          molecule.vibr_energy, i, e);
        }
        else {
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, molecule.diss_energy[e] / (6 * K_CONST_K), molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                          molecule.vibr_energy, i, e);
        }
        break;
    case models_k_diss::model_k_diss_tm_3T_arrh_scanlon:
        if (num_electron_levels == 1) {
            ...
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, 3 * T, molecule.electron_energy, molecule.statistical_weight, molecule.num_electron_levels,
                          molecule.vibr_energy, i, e);
        }
        else {
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, 3 * T, molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                          molecule.vibr_energy, i, e);
        }
        break;
    case models_k_diss::model_k_diss_tm_infty_arrh_scanlon:
        if (num_electron_levels == 1) {
            ...
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, molecule.electron_energy, molecule.statistical_weight, molecule.num_electron_levels,
                          molecule.vibr_energy, molecule.num_vibr_levels, i, e);
        }
        else {
            return k_Arrhenius(T, interaction["diss," + molecule.name + ",Arrh_A,Scanlon"], interaction["diss," + molecule.name + ",Arrh_n,Scanlon"], interaction["diss," + molecule.name + ",Ea,Scanlon"]) * 
                   Z_diss(T, molecule.electron_energy, molecule.statistical_weight, num_electron_levels,
                          molecule.vibr_energy, molecule.num_vibr_levels, i, e);
        }
        break;
      ...
      case models_k_diss::model_k_diss_phys4entry:
          if (e==0) {
              if (molecule.name == "N2" && (interaction.particle1_name == "N" || interaction.particle2_name == "N")) {
                  ...
                  std::string error_string = "No Phys4Entry data for dissociation of " + molecule.name + " in reaction: " + interaction.particle1_name + "+" + interaction.particle2_name;
              throw kappa::ModelParameterException(error_string.c_str());
              }
          }
          else {
...
double kappa::Approximation::k_diss(double T, kappa::Molecule const &molecule, kappa::Interaction const &interaction, int i, kappa::models_k_diss model) {
...
double kappa::Approximation::k_bf_VT(double T, kappa::Molecule const &molecule, int i, int delta_i, int e) {
  return p_k_bf_VT(T, molecule.vibr_energy[e][i], molecule.vibr_energy[e][i + delta_i], molecule.rot_energy[e][i], molecule.num_rot_levels[e][i],
                 molecule.rot_energy[e][i + delta_i], molecule.num_rot_levels[e][i + delta_i], molecule.rot_symmetry, molecule.rigid_rotator);
}

double kappa::Approximation::k_bf_VV(double T, kappa::Molecule const &molecule1, kappa::Molecule const &molecule2, int i, int k, int delta_i, int e1, int e2) {
    return p_k_bf_VV(T, molecule1.vibr_energy[e1][i], molecule1.vibr_energy[e1][i + delta_i], molecule2.vibr_energy[e2][k], molecule2.vibr_energy[e2][k - delta_i],
                   molecule1.rot_energy[e1][i], molecule1.num_rot_levels[e1][i], molecule1.rot_energy[e1][i + delta_i], molecule1.num_rot_levels[e1][i + delta_i], molecule1.rot_symmetry, 
                   molecule2.rot_energy[e2][k], molecule2.num_rot_levels[e2][k], molecule1.rot_energy[e2][k - delta_i], molecule2.num_rot_levels[e2][k - delta_i], molecule2.rot_symmetry);
}

double kappa::Approximation::k_bf_exch(double T, kappa::Molecule const &molecule_before, kappa::Atom const &atom_before,
                                       kappa::Molecule const &molecule_after, kappa::Atom const &atom_after,
                                       kappa::Interaction const &interaction, int i_before, int i_after, int e_before, int e_after) {
  return p_k_bf_exch(T, molecule_before.mass, atom_before.mass, molecule_after.mass, atom_after.mass,
                   molecule_before.diss_energy[e_before], molecule_after.diss_energy[e_after], molecule_before.vibr_energy[e_before][i_before], molecule_after.vibr_energy[e_after][i_after],
                   molecule_before.rot_energy[e_before][i_before], molecule_before.num_rot_levels[e_before][i_before], molecule_before.rot_symmetry,
                   molecule_after.rot_energy[e_after][i_after], molecule_after.num_rot_levels[e_after][i_after], molecule_after.rot_symmetry);
}
...
double kappa::Approximation::k_bf_diss(double T, kappa::Molecule const &molecule, kappa::Atom const &atom1, kappa::Atom const &atom2, int i, int e) {
  return p_k_bf_diss(T, molecule.mass, atom1.mass, atom2.mass, molecule.diss_energy[e], molecule.vibr_energy[e][i], molecule.rot_energy[e][i], molecule.num_rot_levels[e][i], molecule.rot_symmetry);
}

double kappa::Approximation::k_bf_diss(double T, kappa::Molecule const &molecule, kappa::Atom const &atom, int i, int e) {
  return p_k_bf_diss(T, molecule.mass, atom.mass, atom.mass, molecule.diss_energy[e], molecule.vibr_energy[e][i], molecule.rot_energy[e][i], molecule.num_rot_levels[e][i], molecule.rot_symmetry);
}
